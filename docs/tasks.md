...
| 序号 | 任务 | 状态 | 说明 |
|-----|------|-----|------|
| 3.1 | 搭建性能测试框架 | ✅ | `make benchmark` 自动执行，结果保存到 benchmark_results.txt |
| 3.2 | 设计测试场景 | ✅ | 自动生成测试数据脚本，支持小/中/大文件、深层目录、混合内容 |
| 3.3 | 执行基准测试 | ✅ | 2026-02-21 完成基准测试，结果见 benchmark_results.txt |
...
| 3.1 | 搭建性能测试框架 | 2026-02-21 | `make benchmark` 自动执行，结果保存到 benchmark_results.txt (含 real/user/sys) |
| 3.2 | 设计测试场景 | 2026-02-21 | `scripts/generate_test_data.py` 支持标准(~80MB)和压力(~500MB-1GB)两种模式 |
| 3.3 | 执行基准测试 | 2026-02-21 | 压力模式打包耗时 5-30s，减少计时误差，适合压缩算法对比 |
